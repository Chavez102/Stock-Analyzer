...
//	}
	...
	public static HashMap<String, Stock> getBagFor(String StockSymbol) {
		...
		HashMap<String, Stock> map = new HashMap<>(bag.stocklist.size() * 2); // array is 2 times bigger than actual
																				// size of data elements

		for (int i = 0; i < bag.stocklist.size(); i++) {
			Stock stock = new Stock(bag.stocklist.get(i).Stringdate, bag.stocklist.get(i).Stringopen,
					bag.stocklist.get(i).Stringhigh, bag.stocklist.get(i).Stringlow, bag.stocklist.get(i).Stringclose,
...
//   https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=TSLA&interval=5min&apikey=AJTUPL59H6NMOMYH&outputsize=full
			String url = "https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=" + symbol
					+ "&interval=5min&apikey=" + key + "&outputsize=full";
